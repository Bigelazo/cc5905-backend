...
import model.unit.{Character, Fighter, Party, Weapon}
...
class GameState {
  ...
  private val allies: HashMap[Int, Character] = HashMap(
  ...
  private val enemies: HashMap[Int, Character] = HashMap(
  ...
  private val alliesSide: HashMap[Int, Panel] = HashMap(
    1 -> new Panel((1, 1)),
    2 -> new Panel((1, 2)),
    3 -> new Panel((1, 3)),
    4 -> new Panel((2, 1)),
  ...
  private val enemiesSide: HashMap[Int, Panel] = HashMap(
    10 -> new Panel((1, 1)),
    11 -> new Panel((1, 2)),
    12 -> new Panel((1, 3)),
    13 -> new Panel((2, 1)),
  ...
  parties.head.characters = allies.values.toList
  parties(1).characters = enemies.values.toList

  parties.head.panels = alliesSide.values.toList
  parties(1).panels = enemiesSide.values.toList
